...
interface FormCreatePlaylistProps {
  title: string;
...
export const FormCreatePlaylist: React.FC<FormCreatePlaylistProps> = ({
  title: externalTitle,
  ...
  const [title, setInternalTitle] = useState(externalTitle);
  const [error, setError] = useState('');
  const [isFocused, setIsFocused] = useState(false);
  const [labelAnim] = useState(new Animated.Value(externalTitle ? 1 : 0));

  // Sincroniza el estado interno cuando cambia el título externo
  useEffect(() => {
  ...
  const handleTitleChange = (text: string) => {
    setInternalTitle(text);
    setExternalTitle(text);
    setError('');
  };
